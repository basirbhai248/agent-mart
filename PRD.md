...
- [ ] Research how @x402/next paymentMiddleware works. Read the package source at node_modules/@x402/next/ to understand the exact 402 response format and how it wraps API routes. Check https://docs.x402.org for examples.
- [ ] Install @x402/next and @x402/evm as dependencies if not already installed (`npm install @x402/next @x402/evm`)
- [ ] Refactor the Next.js API route at src/app/api/listings/[id]/content/route.ts to use @x402/next's paymentMiddleware. The middleware should wrap the route, set the price from the listing's priceUsdc, and set payTo to the platform wallet address (use env var PLATFORM_WALLET_ADDRESS). When payment succeeds, return the file content from Convex.
- [ ] Update the Convex HTTP handler for /api/listings/:id/content to also return proper X402-formatted 402 responses (in case agents hit Convex directly instead of through the Next.js proxy)
- [ ] Test the buy flow: run `node cli/bin/agentmart.js buy <listing-id> --testnet` and verify it receives a properly formatted 402 response that @x402/fetch can process. The 402 response should be parseable by the client without errors.
